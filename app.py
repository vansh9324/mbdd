...
def load_data():
    ...
    # drop blank / nan kshetras
    df = df[df["Kshetra"].str.strip().str.lower().ne("nan") & df["Kshetra"].str.strip().ne("")]
    return df
...
# top karyakarta per kshetra
top_by_ksh = (df.groupby([REGION, WORKER]).size()
...
top_workers = (df.groupby([WORKER, WID]).size()
                 .reset_index(name="Registrations")
                 .sort_values("Registrations", ascending=False)
                 .head(10))
...
# ── Two-column charts (state & kshetra) ────────────────────
left, right = st.columns(2)

with left:
    st.markdown("<h2 style='color:#ffa600;'>📊 Registrations by State</h2>",
                unsafe_allow_html=True)
...
# ── Marquee Top-10 karyakartas ─────────────────────────────
ticker = "  |  ".join(
    f"<span style='color:#ccc;'>ID:{r[WID]}</span> – "
